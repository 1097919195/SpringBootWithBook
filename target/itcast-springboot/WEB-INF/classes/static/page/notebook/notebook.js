					...
					if (Utils.parseOptResult(result)) {//token仍存在，未失效
						if(result.state==0){ //查询笔记本成功
							...
							for (var i = 0;i <result.data.length;i++) {
								...
								sli += '<div class="msg-time"><i class="fa fa-book" title="online" rel="tooltip-bottom ">'+result.data[i].cn_notebook_name+'</i>';
								sli +='</div>';
								sli += '</a></li>';
					...
					if (Utils.parseOptResult(result)) {//token仍存在，未失效
						if(result.state==0){ //查询笔记成功
							for (var i = 0;i <result.data.length;i++) {
								...
								sli +='<a href="javascript:void(0);" class="clearfix" style="line-height:20px;"';
								sli += '<div class="blue" style="line-height:18px;padding:5px 15px;" id="createrName" >'+'</div>';
								sli += '<div class="msg-time"><i class="fa fa-file-text-o" title="online" rel="tooltip-bottom ">'+result.data[i].cn_note_title+'</i></div>';
								sli += '</a></li>';
								$("#note_li").append(sli);
					}
					...
					success:function(result){
						if (Utils.parseOptResult(result)) {//token仍存在，未失效
							if(result.state==0){ //
								...
								sli +='<a href="javascript:void(0);" class="clearfix" style="line-height:20px;"';
								sli += '<div class="blue" style="line-height:18px;padding:5px 15px;" id="createrName" >'+'</div>';
								sli += '<div class="msg-time"><i class="fa fa-book" title="online" rel="tooltip-bottom ">'+result.data.cn_notebook_name+'</i></div>';
								sli += '</a></li>';
								$("#notebook_li").append(sli);
				...
				success:function(result){
					if (Utils.parseOptResult(result)) {//token仍存在，未失效
						if(result.state==0){ //
							...
							sli +='<a href="javascript:void(0);" class="clearfix" style="line-height:20px;"';
							sli += '<div class="blue" style="line-height:18px;padding:5px 15px;" id="createrName" >'+'</div>';
							sli += '<div class="msg-time"><i class="fa fa-file-text-o" title="online" rel="tooltip-bottom ">'+result.data.cn_note_title+'</i></div>';
							sli += '</a></li>';
							$("#note_li").append(sli);
								...
								if(result.state==0){
									...
									for(var i=0;i<result.data.length;i++){
										if(result.data[i].cn_notebook_id!=cn_notebook_id){
											...
											sl += '<div class="msg-time"><i class="fa fa-file-text-o" title="online" rel="tooltip-bottom ">'+result.data[i].cn_note_title+'</i></div>';
											sl += '</a></li>';
											$("#note_li").append(sl);
										}
								}else{
